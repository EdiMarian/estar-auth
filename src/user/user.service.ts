...
export class UserService {
  ...
  async getUserProfileImage(connected: any): Promise<string> {
  ...
  async getUserTokens(addresses: UserAddress[]): Promise<any[]> {
    ...
    if (chainAddress) {
      ...
      try {
        const tokensFound = await this.apiProvider.doGetGeneric(
          `accounts/${chainAddress.address}/tokens?${params.toString()}`,
        );
  ...
  async getUserNfts(
    ...
    if (chainAddress) {
      ...
      try {
        const nftsFound = await this.apiProvider.doGetGeneric(
          `accounts/${chainAddress.address}/nfts?${params.toString()}`,
        );
        return nftsFound;
      } catch (error) {
        this.logger.error(error);
